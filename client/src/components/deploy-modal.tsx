...
export default function DeployModal({ isOpen, onClose }: DeployModalProps) {
  ...
  const [isGitHubSetupModalOpen, setIsGitHubSetupModalOpen] = useState(false); // State for the new modal
  ...
  // New state and handler for the GitHub Setup Modal
  const [showGitHubSetup, setShowGitHubSetup] = useState(true); // Initially true to show for new users
  ...
  return (
    <>
      ...
      {/* Render the GitHub Setup Modal */}
      <GitHubSetupModal
        isOpen={showGitHubSetup && isOpen} // Only show if the main modal is open and it's the first time
        onClose={() => {
          setShowGitHubSetup(false); // Don't show again for this session
          // Optionally, you might want to refetch GitHub status after setup is complete
